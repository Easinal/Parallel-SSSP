...
radiuses=("0" "1" "2" "3" "4")
xs=("1" "2" "3" "4" "5")
graphs=("RoadUSA_sym_wgh" "Germany_sym_wgh" "hugebubbles-00020" "road-asia-osm" "hugetrace-00020")
for graph in ${graphs[@]}; do
    ...
    for radius in ${radiuses[@]}; do
        for x in ${xs[@]}; do
            ./sssp -i ${folderOrigin}/${graph}.adj -c ${folderContract}/${graph}.adj -w -v -a rho-stepping -r ${radius} -x ${x} > ~/Parallel-SSSP/${graph}/delta_${radius}_${x}.txt
        done
    done
done
