...
for graph in ${graphs[@]}; do
    for step in ${steps[@]}; do
        ./sssp -i ${folderOrigin}/${graph}_sym_wgh.adj -f ${folderContract}/${graph}_sym_wgh.adj -d ${folderStar}/${graph}_sym_wgh.adj -p ${step} -w -v -c -a rho-stepping > ${graph}/rho_${step}.txt
        ./sssp -i ${folderOrigin}/${graph}_sym_wgh.adj -f ${folderContract}/${graph}_sym_wgh.adj -d ${folderStar}/${graph}_sym_wgh.adj -p ${step} -w -v -c -a delta-stepping > ${graph}/delta_${step}.txt
    done
done
